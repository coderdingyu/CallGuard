...
def run_curl_download(url: str, target_path: Path, token: str | None) -> None:
    ...
    args = ["curl.exe", "-L", "--fail", "--retry", "3", "--output", str(target_path)]
    if token:
...
def download_teleantifraud(data_root: Path, priority_only: bool, method: str) -> None:
...
def main() -> None:
    ...
    if args.dataset == "teleantifraud":
        download_teleantifraud(data_root, priority_only=not args.all_files, method=args.method)
